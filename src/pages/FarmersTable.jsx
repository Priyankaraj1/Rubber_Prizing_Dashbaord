...
const tableHeaderBg = isDark ? theme.palette.grey[900] : theme.palette.grey[800];
const tableRowBg = isDark ? theme.palette.background.default : "#ffffff";
const tableText = theme.palette.text.primary;
const tableBorder = theme.palette.divider;
 ...
 const {
  ...
  shouldUnregister: false, 
    ...
    useEffect(() => {
  if (openEdit && selectedFarmer) {
    reset({
      name: selectedFarmer.name ?? "",
      gender: selectedFarmer.gender ?? "",
      address: selectedFarmer.address ?? "",
      total_agri_area: selectedFarmer.total_agri_area ?? "",
      rubber_area_mature: selectedFarmer.rubber_area_mature ?? "",
      rubber_area_immature: selectedFarmer.rubber_area_immature ?? "",
      farm_type: selectedFarmer.farm_type ?? "",
      latitude: selectedFarmer.latitude ?? "",
      longitude: selectedFarmer.longitude ?? "",
      harvesting_tree: selectedFarmer.harvesting_tree ?? "",
      harvesting_trade: selectedFarmer.harvesting_trade ?? "",
      yield: selectedFarmer.yield ?? "",
    });
  }
}, [openEdit, selectedFarmer, reset]);
...
>{farmer.rubber_area_immature}</td>
                    <td
  style={{
    padding: "10px 12px",
    fontSize: "0.9rem",
...
>{farmer.harvesting_trade} kg</td> */}
  ...
  onClick={() =>
    navigate(`/farmer-quality/${farmer.id}`, {
      state: { farmerName: farmer.name }   // ← ADD THIS
    })
  }
>
  <AddChart />
...
{/* 
  ...
  <TextField label="Rubber area immature" {...register("rubber_area_immature")} fullWidth />
                <TextField label="Farm type" {...register("farm_type")} fullWidth />

                {/* <TextField label="LATITUDE" {...register("latitude")} fullWidth />
                <TextField label="LONGITUDE" {...register("longitude")} fullWidth /> */}

                <TextField
  label="Harvesting tree"
  {...register("harvesting_tree")}
  fullWidth
  InputLabelProps={{ shrink: true }}
/>

<TextField
  label="Harvesting trade"
  {...register("harvesting_trade")}
  fullWidth
  InputLabelProps={{ shrink: true }}
/>
